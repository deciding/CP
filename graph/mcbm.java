...
class mcbm {
  ...
  private static int Aug(int l) {
    ...
    while (it.hasNext()) { // either greedy assignment or recurse
      ...
      //match.get(right) == -1 is the base case, always stop at unmatched right
      //Aug(match.get(right)) always runs for left
...
*/
    ...
    match.addAll(Collections.nCopies(V, -1));//match is used globally for all iterations
    for (int l = 0; l < V_l; l++) {
      visited = new Vector < Integer > ();// visited is used in every iteration
      visited.addAll(Collections.nCopies(V_l, 0));
      MCBM += Aug(l);//stop when there are no augmenting path
    }
