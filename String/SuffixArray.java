import java.util.*;
//RA->countingSort(k)->SA->countingSort(0)->SA->RA->...
class sa {
  Scanner scan;
  ...
  void countingSort(int k) {
    ...
    for (i = 0; i < n; i++)    //(in dictionary)don't care order !         // count the frequency of each rank
      c[i + k < n ? RA[i + k] : 0]++;//c[0]->c[1]:count of all i+k>=n !
    for (i = sum = 0; i < maxi; i++) {
      int t = c[i]; c[i] = sum; sum += t;
    }
    for (i = 0; i < n; i++)               // shuffle the suffix array if necessary
      //key sentence!
      // for k:in table top down order !
      // for 0:c[...]++ is to assign unique rank to non-unique rank RA
      //      RA[SA[i]] means that, if SA[i] got higher rank i in countingSort(k)
      //      it will get unique new rank earlier !
    ...
    for (k = 1; k < n; k <<= 1) {            // repeat sorting process log n times
      ...
      for (i = 1; i < n; i++)                         // compare adjacent suffices
        tempRA[SA[i]] =      // if same pair => same rank r; otherwise, increase r
          //!because of '$' SA[i]+k will never out of bound when first condition satisfied!
